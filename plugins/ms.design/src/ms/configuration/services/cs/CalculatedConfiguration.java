...
public class CalculatedConfiguration {
	...
	public CalculatedConfiguration(Result r) {
		...
		configList = new ArrayList<CSConfiguration>();
	}
	...
	// represents the content of a Modes & State CSConfiguration but does not
	// corresponds to an item of the capella data model.
	// used only for the calculation of configurations associated to situation
	// object
	...
	public static class LightConfiguration {
		...
		public LightConfiguration(CSConfiguration initial_conf) // construction
																// from an
																// existing
																// CSConfiguration
		{
	...
	public class BooleanExpr {
		...
		boolean isComposite;
		boolean isExcluded;
		boolean valStatus; // applicable a un noeud composite : mis � 1 si la
							// configuration r�sultante sur ce noeud est valide

		List<BooleanExpr> children;

		ConfList linked_configurations; // liste des cnofigurations associ�es �
										// ce noeud. (liste s�par�es par des OU)
	...
	private LightConfiguration compute_XOR(LightConfiguration... cfgs) {
		...
		rcfg = new LightConfiguration("Computed configuration");
	...
	private static boolean CHK_ERR_Alloc_on_exluded_comp(LightConfiguration... cfgs) {
		...
		// parcours deux � deux les cfgs[ ] et verifie si un �l�ment inclus dans
		// le premier n'est pas exlu dans le deuxieme et invers�ment
		if (cfgs.length >= 2) {
			...
			for (int i = 0; i < cfgs.length; i++) {
				for (int j = 0; j < cfgs.length; j++) {
					...
					// recupere la liste des fonctinos allou�e sur les
					// composants exclus
					...
					// conserve seulement les fonctions inclues et alloues sur
					// composants exclus
					...
					for (int i_err = 0; i_err < F_allocatedOnExclComp.size(); i_err++) {
						...
						if (F_allocatedOnExclComp.get(i_err).getAllocationBlocks().get(0) != null)
							erroneous_comp_name = F_allocatedOnExclComp.get(i_err).getAllocationBlocks().get(0)
									.getName();

						ErrorListText.add(" Function '" + F_allocatedOnExclComp.get(i_err).getName()
								+ "' is included and allocated on an excluded component '" + erroneous_comp_name
								+ "' ");
						ErrorListRuleType.add("INCL_EXCL_INVOLVED_ELEMENT");
						System.out.println(" Function '" + F_allocatedOnExclComp.get(i_err).getName()
								+ "' is included and allocated on an excluded component '" + erroneous_comp_name
								+ "' ");
					}
					incl_Fs.clear();
			...
			for (int i = 0; i < cfgs.length; i++) {
				for (int j = 0; j < cfgs.length; j++) {
					...
					for (int ifc = 0; ifc < incl_CFs.size(); ifc++) {
						...
						// recupere la liste des fonctions allou�e sur les
						// composants exclus
						List<AbstractFunction> F_allocatedOnExclComp = new ArrayList<AbstractFunction>();
						...
						// conserve seulement les fonctions inclues et alloues
						// sur composants exclus
						F_allocatedOnExclComp.retainAll(incl_Fs);
			...
			for (int i = 0; i < cfgs.length; i++) {
				for (int j = 0; j < cfgs.length; j++) {
					...
					for (int ifc = 0; ifc < incl_CFs.size(); ifc++) {
						...
						// list of erroneous elements : function involved and
						// excluded :
						ArrayList<AbstractFunction> common_inclexclFunctions = new ArrayList<AbstractFunction>(
								common_inclFunctions);
						common_inclexclFunctions.retainAll(cfgs[j].excl_lF); // keep
																				// only
																				// the
																				// elements
																				// included
						...
						for (int i_err = 0; i_err < common_inclexclFunctions.size(); i_err++) {
							rule_failed = true;
							ErrorListText.add(" Function '" + common_inclexclFunctions.get(i_err).getName()
									+ "' is involed in a functional chain '" + incl_CFs.get(ifc).getName()
									+ "' and excluded");
							ErrorListRuleType.add("INCL_EXCL_INVOLVED_ELEMENT");
							System.out.println(" Function '" + common_inclexclFunctions.get(i_err).getName()
									+ "' is involed in a functional chain '" + incl_CFs.get(ifc).getName()
									+ "' and excluded");
						}
					...
					for (int ifc = 0; ifc < excl_CFs.size(); ifc++) {
						...
						// list of erroneous elements : function involved in
						// exluced fc and included :
						ArrayList<AbstractFunction> common_inclexclFunctions = new ArrayList<AbstractFunction>(
								common_exclFunctions);
						common_inclexclFunctions.retainAll(cfgs[j].lF); // keep
																		// only
																		// the
																		// elements
																		// included
		...
		if (cfgs.length >= 2) {
			...
			for (int i = 0; i < cfgs.length; i++) {
				for (int j = 0; j < cfgs.length; j++) {
					...
					common_Functions.retainAll(cfgs[j].excl_lF); // keep only
																	// the
																	// elements
																	// included
																	// in excl_F
					...
					if (true) {
						for (int i_err = 0; i_err < common_Functions.size(); i_err++) {
							rule_failed = true;
							ErrorListText.add(" Function both included and excluded : '"
									+ common_Functions.get(i_err).getName() + "' ");
							ErrorListRuleType.add("INCL_EXCL_ELEMENT");
							System.out.println(" Function both included and excluded : '"
									+ common_Functions.get(i_err).getName() + "' ");
						}

						for (int i_err = 0; i_err < common_CFs.size(); i_err++) {
							rule_failed = true;
							ErrorListText.add(" Functional chain both included and excluded : '"
									+ common_CFs.get(i_err).getName() + "' ");
							ErrorListRuleType.add("INCL_EXCL_ELEMENT");
							System.out.println(" Functional chain both included and excluded : '"
									+ common_CFs.get(i_err).getName() + "' ");

						}
						for (int i_err = 0; i_err < common_Cs.size(); i_err++) {
							rule_failed = true;
							ErrorListText.add(" Component both included and excluded : '"
									+ common_Cs.get(i_err).getName() + "' ");
							ErrorListRuleType.add("INCL_EXCL_ELEMENT");
							System.out.println(" Component both included and excluded : '"
									+ common_Cs.get(i_err).getName() + "' ");
	...
	private static LightConfiguration compute_AND(LightConfiguration... cfgs) {
		LightConfiguration rcfg = new CalculatedConfiguration.LightConfiguration("Computed Configuration");
	...
	// r�alise l'operation ET sur deux ou plus de deux listes de configuerations
	private static ConfList Operator_AND(ArrayList<ConfList> operandes) {
		ConfList cl = new ConfList("Computed Configuration");
		// cl = operandes.get(0);
		// realise le d�veloppement du ET sur les listes de configurations de
		// chaque operande, 2 a 2 :
		int k_oper = 0;

		if (operandes.size() == 1) {
			cl = operandes.get(0); // cas d'un ET d'un seul �l�ment
		}
		if (operandes.size() > 1) {
			while (k_oper < operandes.size() - 1) {
				// copie les deux premiers op�randes, ou, pour le premier, le
				// r�sultat de l'iteration precedente pour l'it�ration actuelle
	...
	// si le NOT s'applique � une configuration calcul�e, contenant une liste de
	// configurations potentielles
	// s�par�es par des OU, alors, fusionne ces configurations potentielles en
	...
	// r�alise l'operation XOR sur deux ou plus de deux listes de
	// configuerations
	private static ConfList Operator_XOR(ArrayList<ConfList> operandes) {
		...
		int k_oper = 0;
		if (operandes.size() == 1) {
			cl = operandes.get(0); // cas d'un OU d'un seul �l�ment
		}
		if (operandes.size() > 1) {
			while (k_oper < operandes.size() - 1) {
				// copie les deux premiers op�randes, ou, pour le premier, le
	...
	// explore r�cursivement l'arborescence, et calcule la liste de
	// configurations
	// r�sultantes en remontant depuis les noeuds feuilles.
	private static ConfList Explore_be(BooleanExpr be) {
		...
		if (be.isComposite) {
			...
			// fin de parcourt de tous les enfants, r�aliser la compilation sur
			// ce noeud
			...
			for (int i = 0; i < be.linked_configurations.Cl.size(); i++) {
				...
				for (int j = 0; j < be.linked_configurations.Cl.get(i).lF.size(); j++) {
					// if (be.linked_configurations.Cl.get(i).isExcluded)
					// System.out.print('x');
			...
			return be.linked_configurations;
				...
				// des configurations associ�es � ce M/S
		{
			// System.out.println(" % ");
			...
			for (int i = 0; i < be.linked_configurations.Cl.size(); i++) {
				// cree un conflist a partir des configurations associ�e a la
	...
	private boolean initializeBoolExprTree(BooleanExpr parent_be, EObject be_object) {
		for (EObject jObj : be_object.eContents()) {
			...
			if (jObj instanceof InStateExpression) {
				...
				// find 0, 1 or several linked configuration(s) to this mode /
				// state :
	...
	public ConfList Calculate() {
		if (owning_result != null)
			if (owning_result.getSituation().size() > 0)
		...
		java.lang.System.out.println("config list ok");
		...
		for (EObject iObj : owning_situation.eContents()) {
			...
			java.lang.System.out.println("start initialize");
		...
		java.lang.System.out.println("start explore");
